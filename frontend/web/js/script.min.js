...
$('#registration-form').submit(function(){gtag('event','register',{'event_category':'form','event_action':'register',});yaCounter53666866.reachGoal('register')});$('#reset_token_form').submit(function(e){e.preventDefault();$.ajax({type:'POST',url:'/reset-password/send-token',dataType:'json',data:$(this).serialize(),success:function(data){if(data!=!1){$('.jsForgotPassModal').fadeOut(1);$('.jsModalSuccess').fadeIn();$('.jsModalSuccess p')[0].innerText='Сообщение с дальнейшими инструкциями было отправлено на ваш Email'}}})});$('.jsClickShowPhone').click(function(){$(this).fadeOut(1);$('.jsShowPhone').fadeIn(1);let url='';if($(this).attr('data-type')==='vacancy'){url='/vacancy/default/click-phone'}else if($(this).attr('data-type')==='resume'){url='/resume/default/click-phone'}else if($(this).attr('data-type')==='company'){url='/company/default/click-phone'}
$.ajax({type:'POST',url:url,data:{csrf:$('meta[name=csrf-token]').attr('content'),id:$(this).attr('data-id')},success:function(response){}})});let btnAcceptTop=$('.jsAcceptScroll').offset().top;function btnAcceptScroll(){if(($(window).scrollTop()+$(window).height())>(btnAcceptTop-150)){$('.jsAcceptScrollFixed').fadeOut(300)}else{$('.jsAcceptScrollFixed').fadeIn(300)}}
btnAcceptScroll();$(window).scroll(function(){btnAcceptScroll()})});if($('.jsCitiesSelect').length>0){$('.jsCitiesSelect').select2({placeholder:"Выберите город",allowClear:!0})}
...
if($('.jsCountryHeaderSelect').length>0){$('.jsCountryHeaderSelect').select2({placeholder:"Выберите Страну",allowClear:!0}).on('change',function(){$.ajax({type:"POST",url:"/main_page/default/select-country",data:{country:$(this).val(),_csrf:$('meta[name=csrf-token]').attr("content")},success:function(result){window.location.reload()}})})}
