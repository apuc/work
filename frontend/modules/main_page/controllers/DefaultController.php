...
//            return $this->redirect('/');
            ...
            $cities = Yii::$app->cache->getOrSet("main_page_cities_$current_country->slug", function () use ($current_country) {
                return City::find()
                    ->select([
                        ...
                        'slug'])
                    ->joinWith('region')
                    ->where([City::tableName().'.status' => 1, Region::tableName().'.country_id'=>$current_country->id])
    ...
    {
        $cities_dnr = Yii::$app->cache->getOrSet('cities_dnr', function () {
            return City::find()->where(['status'=> City::TYPE_SHOWN, 'region_id'=>21])->all();
        });
        $cities_lug = Yii::$app->cache->getOrSet('cities_lug', function () {
            return City::find()->where(['status'=> City::TYPE_SHOWN, 'region_id'=>19])->all();
        });
